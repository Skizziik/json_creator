...
class Store {
  ...
  addBlankChunk(projectName, catId) {
    const data = this._load(projectName);
    const cat = data.categories.find(c => c.id === catId);
    if (!cat) throw new Error('Category not found');
    const newChunk = {
  ...
  updateChunk(projectName, catId, chunkUid, updates) {
    const data = this._load(projectName);
    const cat = data.categories.find(c => c.id === catId);
    if (!cat) throw new Error('Category not found');
  ...
  deleteChunk(projectName, catId, chunkUid) {
    const data = this._load(projectName);
    const cat = data.categories.find(c => c.id === catId);
    if (!cat) throw new Error('Category not found');
  ...
  duplicateChunk(projectName, catId, chunkUid) {
    const data = this._load(projectName);
    const cat = data.categories.find(c => c.id === catId);
    if (!cat) throw new Error('Category not found');
    const ch = cat.chunks.find(c => c._uid === chunkUid);
  ...
  importJSON(projectName, jsonArray, categoryName) {
    ...
    let data;
    try { data = this._load(projectName); } catch { data = this.createProject(projectName); }
